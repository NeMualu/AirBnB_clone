...
class HBNBCommand(cmd.Cmd):
    ...
    def display_errors(self, line, num_of_args):
        """Displays error messages to the user

        Args:
            line (any): gets user input using the command line
    ...
    def do_create(self, line):
        ...
        """
        if self.display_errors(line, 1) == 1:
            return
        args = line.split(" ")

        """
        args[0] contains class name, create a new instance
        of that class updates 'updated_at' attribute,
        and saves into the JSON file
        """
        obj = eval(args[0])()
        obj.save()
    ...
    def do_show(self, line):
        ...
        """
        if self.display_errors(line, 2) == 1:
            return
        args = line.split()
        d = storage.all()
        if args[1][0] == '"':
    ...
    def do_update(self, line):
        ...
        """
        if self.display_errors(line, 4) == 1:
            return
        args = line.split()
        d = storage.all()
        ...
        if attr_k in class_attr.keys():
            try:
                ...
                print("Entered the wrong value type")
                return
        setattr(d[key], attr_k, attr_v)
        storage.save()

    def count_instances(self, class_n):
        """
    ...
    def default(self, line):
        """Method to take care of the following commands:
        <class name>.all()
        <class name>.count()
        <class name>.show(<id>)
        ...
        Description:
            Creates list representations of functional models
